...
import { LayoutDashboard, Users, LineChart, FileText, Settings, ShieldAlert, GraduationCap } from 'lucide-react';

const NAV_ITEMS = [
  { href: '/coordinator/dashboard', label: 'Dashboard', icon: LayoutDashboard },
  { href: '/coordinator/analytics', label: 'Analytics', icon: LineChart },
  { href: '/coordinator/students', label: 'Student Monitoring', icon: ShieldAlert },
  { href: '/coordinator/interventions', label: 'Interventions', icon: Settings },
  ...
  return (
    <aside className="fixed left-0 top-0 bottom-0 w-64 bg-white border-r border-[#E5E7EB] flex flex-col z-30">
      ...
      <div className="flex items-center gap-3 px-6 h-16 border-b border-[#E5E7EB] shrink-0 bg-[#F9FAFB]/50">
        ...
        <div>
          <p className="text-[15px] font-bold text-[#111827] tracking-tight leading-tight">EduShield</p>
          <p className="text-[11px] text-[#6B7280] font-medium leading-tight uppercase tracking-wider">Coordinator</p>
        </div>
      </div>

      {/* Navigation */}
      <nav className="flex-1 px-4 py-6 space-y-1 overflow-y-auto">
        <p className="text-[10px] font-bold text-[#6B7280] uppercase tracking-widest px-2 mb-3">Menu</p>
        {NAV_ITEMS.map((item) => {
      ...
      <div className="p-5 border-t border-[#E5E7EB] shrink-0 bg-[#F9FAFB]">
        <div className="flex items-center gap-3 mb-4">
          ...
          <div className="min-w-0">
            <p className="text-sm font-bold text-[#111827] truncate">Dr. A. Coordinator</p>
